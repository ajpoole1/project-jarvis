...
4. Reads and clears the Gmail digest queue from SQLite (briefing owns the 7am slot — no separate 9am digest)
...
Fired by cron at 7:00am daily via `scripts/cron_briefing.sh`. Posts directly to Discord via webhook — does not go through OpenClaw.
...
```bash
/mnt/c/Users/aaron/Documents/python/project-jarvis/scripts/cron_briefing.sh
```

## Notes

- Google News RSS links are Google redirect URLs (unresolvable without browser cookies). Links are formatted as `([Publisher Name](url))` Discord markdown using the source name from the feed.
- Weather silently drops if `wttr.in` is unreachable — briefing continues without it.
- All data sources are independent; any single failure is caught and skipped.

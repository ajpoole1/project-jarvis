...
Relocated from root `CLAUDE.md` (2026-07-06). This file is the authoritative home for
architecture narrative, security model, repo tour, external skills, domain rules, and
phase status. The root `CLAUDE.md` points here via the doc map.
...
Full how-to: `docs/guides/SKILL_GUIDE.md` (authoritative — supersedes inline notes here).
...
```
jarvis/
├── CLAUDE.md                  ← interactive-author overlay (thin)
├── CHARTER.md                 ← repo values, stack, QA pairing, lenses
...
│   ├── dev-crew/
│   │   └── persona-mannkusser.md
│   └── dev-loop/
│       ├── BUILD_RUNBOOK.md
│       └── DEV_LOOP_REFERENCE.md
├── .claude/skills/
│   ├── ship/SKILL.md          ← /ship verb
│   ├── qa/SKILL.md            ← /qa verb
│   └── handoff/SKILL.md       ← /handoff verb
...
│   └── dev-loop/              ← Tom QA helpers, dev-loop tooling (checks.sh, start-build.sh, open-pr.sh)
...
└── .github/
    └── workflows/
        ...
        ├── quality.yml        ← delegates to scripts/dev-loop/checks.sh (CI gate)
...
| Python quality | `quality.yml` | Delegates to `scripts/dev-loop/checks.sh` (Python 3.11 + 3.12 matrix) — **CI gate** |
...
Branch strategy: `feature/skill-name` → `main` (protected)

...
def test_call_gemini_qa_success_first_try(monkeypatch):
    ...
    result, reason = call_gemini_qa("key", "sys", "user", fake)
...
def test_call_gemini_qa_exhausts_503_retry_budget_returns_none(monkeypatch):
    ...
    assert fake.calls == 5
    # Reason must name the actual failure signature, not a vague "something broke" —
    # this is what surfaces in the PR comment / Discord ping per AJ's "loud degradation" ask.
    assert "5" in reason
...
def test_call_gemini_qa_unexpected_response_shape_retries_then_fails(monkeypatch):
    ...
    assert result is None
    assert fake.calls == 3
    assert "response shape" in reason.lower()
...
def test_anthropic_model_pinned_to_sonnet_5():
    """AJ specified sonnet-5 explicitly for the fallback — must not drift to
    opus or an ambiguous default that could resolve to an older Sonnet (4.6)."""
    assert _mod.ANTHROPIC_MODEL == "claude-sonnet-5"


def test_call_claude_fallback_uses_pinned_model():
    fake = _FakeRequests([_FakeResponse(200, _claude_body(json.dumps(_VALID_FINDINGS)))])
    call_claude_fallback("key", "sys", "user", fake)
    assert fake.last_payload["model"] == "claude-sonnet-5"


def test_call_claude_fallback_success():
    fake = _FakeRequests([_FakeResponse(200, _claude_body(json.dumps(_VALID_FINDINGS)))])
    result, reason = call_claude_fallback("key", "sys", "user", fake)
    assert result == _VALID_FINDINGS
    assert reason is None
    assert fake.calls == 1
...
def test_call_claude_fallback_does_not_retry_on_503():
    ...
    This is also the terminal-state guarantee AJ asked for: one Gemini
    exhaustion + one failed Claude attempt = stop, don't bounce providers.
    """
...
def test_call_claude_fallback_unrecoverable_json_returns_none():
    fake = _FakeRequests([_FakeResponse(200, _claude_body("total garbage, no braces"))])
    result, reason = call_claude_fallback("key", "sys", "user", fake)
    assert result is None
    assert "JSON" in reason
